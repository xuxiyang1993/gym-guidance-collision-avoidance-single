...
class A2C_Agent:
    def __init__(self, state_size, action_size, num_routes, n_closest, numEpisodes, message_size, lag_values, positions,
                 intersections):
    ...
    def getValidDistance(self, lat, lon, rte, dist, traf, route_keeper):
        ...
        for i in range(len(dist)):
            ...
            if rte == 2:
                ...
                if rte_int == 0:
                    if traf.lon[index] > self.intersections[0][1] and lat > self.intersections[0][0] and lat < \
                            self.intersections[1][0]:
                        return d
    ...
    def train(self):
        ...
        for transitions in episodes:
            ...
            state = np.array([rep[0] for rep in transitions]).reshape(len(transitions),
                                                                      self.state_size * self.lag_values)
            next_state = np.array([rep[3] for rep in transitions]).reshape(len(transitions),
                                                                           self.state_size * self.lag_values)
            ...
            self.model.fit({'states': state}, {'policy_out': advantages, 'value_out': discounted_rewards}, epochs=1,
                           verbose=0)
